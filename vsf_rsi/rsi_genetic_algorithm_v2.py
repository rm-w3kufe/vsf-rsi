#!/usr/bin/env python3
"""
RSI Genetic Algorithm V2 — Tuned version with improvements
Uses Forest concept: population of trees that evolve over generations.

RSI LEVEL 4: AUTO-EVOLUTION (TUNED)
- Reduced selection pressure (tournament size 2)
- Increased mutation rate (0.2)
- Improved fitness function (more factors)
...
FOREST_FILE = EVOLUTION_DIR / "rsi_forest_v2.json"
EVOLUTION_HISTORY_FILE = EVOLUTION_DIR / "rsi_evolution_history_v2.jsonl"
...
class RSIGeneticAlgorithmV2:
    """Tuned genetic algorithm for tree evolution."""
    
    def __init__(self, population_size: int = 10, mutation_rate: float = 0.2):
        """
        Initialize tuned genetic algorithm.
    ...
    def _random_genome(self, predicate_name: str) -> Dict:
        ...
        return {
            ...
            "branches": [
                {
                    "condition": random.choice([
                        ...
                        "ctx_has($ctx, 'active')",
                        "ctx_equals($ctx, 'rate_of_change', 0.1)",
                        "TRUE"
                    ]),
            ...
            "threshold": random.uniform(0.4, 0.95),
    ...
    def evaluate_fitness(self, forest: List[TreeGenome], predicate_name: str) -> List[TreeGenome]:
        ...
        for genome in forest:
            ...
            # Factor 1: Accuracy (from metrics) - 30% weight
            accuracy = self.metrics.get_accuracy(predicate_name, genome.genes.get("threshold", 0.7))
            fitness += accuracy * 0.3
            
            # Factor 2: Complexity penalty (simpler is better) - 15% weight
            complexity = genome.genes.get("complexity", 1)
            fitness += (1.0 / complexity) * 0.15
            
            # Factor 3: Branch diversity (more diverse is better) - 15% weight
            branch_count = len(genome.genes.get("branches", []))
            fitness += min(branch_count / 5, 1.0) * 0.15
            
            # Factor 4: Threshold appropriateness - 15% weight
            threshold = genome.genes.get("threshold", 0.7)
            if 0.6 <= threshold <= 0.8:  # Optimal range
                fitness += 0.15
            elif 0.5 <= threshold <= 0.9:  # Acceptable range
                fitness += 0.10
            else:
                fitness += 0.05
            
            # Factor 5: Depth appropriateness - 10% weight
            depth = genome.genes.get("depth", 1)
            if 2 <= depth <= 3:  # Optimal depth
                fitness += 0.10
            elif 1 <= depth <= 4:  # Acceptable depth
                fitness += 0.07
            else:
                fitness += 0.03
            
            # Factor 6: Condition variety - 15% weight
            conditions = [b.get("condition", "") for b in genome.genes.get("branches", [])]
            unique_conditions = len(set(conditions))
            fitness += min(unique_conditions / 4, 1.0) * 0.15
    ...
    def select_parents(self, forest: List[TreeGenome], method: str = "tournament") -> List[TreeGenome]:
        ...
        if method == "tournament":
            # Tournament selection with size 2 (reduced pressure)
            for _ in range(len(forest) // 2):
                # Select 2 random candidates (reduced from 3)
                candidates = random.sample(forest, min(2, len(forest)))
                # Select best
    ...
    def mutate(self, genome: TreeGenome) -> TreeGenome:
        ...
        if mutation_type == "threshold":
            ...
            new_threshold = old_threshold + random.uniform(-0.15, 0.15)
            new_threshold = max(0.1, min(0.95, new_threshold))  # Clamp
            ...
            # Mutate depth
            old_depth = mutated_genes.get("depth", 1)
            new_depth = old_depth + random.choice([-1, 1])
            new_depth = max(1, min(4, new_depth))  # Clamp
            mutated_genes["depth"] = new_depth
        
        elif mutation_type == "condition":
            # Add or remove a branch
            if "branches" in mutated_genes:
                if len(mutated_genes["branches"]) < 6 and random.random() < 0.5:
                    # Add branch
                    mutated_genes["branches"].append({
                        "condition": random.choice([
                            "ctx_equals($ctx, 'value', 0.5)",
                            "ctx_equals($ctx, 'value', 0.7)",
                            "ctx_equals($ctx, 'value', 0.9)",
                ...
                elif len(mutated_genes["branches"]) > 2:
                    # Remove branch
                    remove_idx = random.randint(0, len(mutated_genes["branches"]) - 1)
    ...
    def _maintain_diversity(self, forest: List[TreeGenome]) -> List[TreeGenome]:
        """Maintain diversity in forest."""
        # Remove duplicates based on genes
        seen_genes = set()
        unique_forest = []
        
        for genome in forest:
            # Create hashable representation of genes
            genes_hash = json.dumps(genome.genes, sort_keys=True)
            
            if genes_hash not in seen_genes:
                seen_genes.add(genes_hash)
                unique_forest.append(genome)
...
# ── CLI Interface ────────────────────────────────────────────────────
def main():
    """CLI interface for RSI genetic algorithm V2."""
    import argparse
    
    parser = argparse.ArgumentParser(description="RSI Genetic Algorithm V2")
    ...
    if args.command == "evolve":
        ga = RSIGeneticAlgorithmV2(population_size=args.population)
